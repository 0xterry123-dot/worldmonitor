...
export class CountryIntelModal {
  ...
  public showLoading(): void {
    ...
    this.headerEl.innerHTML = `
      ...
      <span class="country-name">${i18n.t('countryIntel.identifyingCountry')}</span>
    `;
    this.contentEl.innerHTML = `
      <div class="intel-brief-section">
        <div class="intel-brief-loading">
          ...
          <span class="intel-loading-text">${i18n.t('countryIntel.locatingRegion')}</span>
        </div>
      </div>
    `;
  ...
  public show(country: string, code: string, score: CountryScore | null, signals?: ActiveSignals): void {
    ...
    if (score) {
      html += `
        <div class="cii-section">
          <div class="cii-label">${i18n.t('countryIntel.instabilityIndex')} ${this.scoreBar(score.score)}</div>
    ...
    chips.push(`<span class="signal-chip stock-loading">📈 ${i18n.t('countryIntel.loadingIndex')}</span>`);
    html += `<div class="active-signals">${chips.join('')}</div>`;

    html += `<div class="country-markets-section"><span class="intel-loading-text">${i18n.t('countryIntel.loadingMarkets')}</span></div>`;
    ...
    html += `
      <div class="intel-brief-section">
        <div class="intel-brief-loading">
          ...
          <div class="intel-skeleton"></div>
          <div class="intel-skeleton short"></div>
          <span class="intel-loading-text">${i18n.t('countryIntel.generatingBrief')}</span>
        </div>
      </div>
    `;
  ...
  public updateBrief(data: CountryIntelData): void {
    ...
    if (data.error) {
      briefSection.innerHTML = `<div class="intel-error">${i18n.t('countryIntel.unableToGenerateBrief')}. ${escapeHtml(data.error)}</div>`;
      return;
    }
    ...
    briefSection.innerHTML = `
      ...
      <div class="intel-footer">
        ${data.cached ? `<span class="intel-cached">📋 ${i18n.t('countryIntel.cached')}</span>` : `<span class="intel-fresh">✨ ${i18n.t('countryIntel.fresh')}</span>`}
  ...
  public updateMarkets(markets: PredictionMarket[]): void {
    ...
    if (markets.length === 0) {
      section.innerHTML = `<span class="intel-loading-text" style="opacity:0.5">${i18n.t('countryIntel.noPredictionMarkets')}</span>`;
      return;
    }
    ...
    section.innerHTML = `<div class="markets-label">📊 ${i18n.t('countryIntel.predictionMarkets')}</div>${items}`;
  }
