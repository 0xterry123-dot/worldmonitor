...
export class StatusPanel {
  ...
  constructor() {
    ...
    this.element.innerHTML = `
      <button class="status-panel-toggle" title="${i18n.t('status.tooltip')}">
        <span class="status-icon">◉</span>
      </button>
      <div class="status-panel hidden">
        <div class="status-panel-header">
          <span>${i18n.t('status.title')}</span>
          <button class="status-panel-close">${i18n.t('modals.close')}</button>
        </div>
        <div class="status-panel-content">
          <div class="status-section">
            <div class="status-section-title">${i18n.t('status.dataFeeds')}</div>
            <div class="feeds-list"></div>
          </div>
          <div class="status-section">
            <div class="status-section-title">${i18n.t('status.apiStatus')}</div>
            <div class="apis-list"></div>
          </div>
          <div class="status-section">
            <div class="status-section-title">${i18n.t('status.storage')}</div>
            <div class="storage-info"></div>
          </div>
        </div>
        <div class="status-panel-footer">
          <span class="last-check">${i18n.t('status.updatedJustNow')}</span>
        </div>
      </div>
    `;
  ...
  private updateDisplay(): void {
    ...
    lastCheck.textContent = `${i18n.t('status.updated')} ${this.formatTime(new Date())}`;
  }
  ...
  private async updateStorageInfo(container: Element): Promise<void> {
    try {
      if ('storage' in navigator && 'estimate' in navigator.storage) {
        ...
        container.innerHTML = `<div class="status-row">${i18n.t('status.storageUnavailable')}</div>`;
      }
    } catch {
      container.innerHTML = `<div class="status-row">${i18n.t('status.storageUnavailable')}</div>`;
    }
