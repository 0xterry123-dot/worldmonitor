...
export class InsightsPanel extends Panel {
  ...
  constructor() {
    super({
      id: 'insights',
      title: i18n.t('panels.insights'),
      showCount: false,
      infoTooltip: `
        <strong>${i18n.t('insights.title')}</strong><br>
        • <strong>${i18n.t('insights.worldBrief')}</strong><br>
        • <strong>${i18n.t('insights.sentiment')}</strong><br>
        • <strong>${i18n.t('insights.velocity')}</strong><br>
        • <strong>${i18n.t('insights.focalPoints')}</strong><br>
        <em>${i18n.t('insights.desktopOnly')}</em>
      `,
    });
  ...
  public async updateInsights(clusters: ClusteredEvent[]): Promise<void> {
    ...
    if (clusters.length === 0) {
      this.setContent(`<div class="insights-empty">${i18n.t('insights.waitingForData')}</div>`);
      return;
    }
    ...
    try {
      ...
      if (importantClusters.length === 0) {
        this.setContent(`<div class="insights-empty">${i18n.t('insights.noBreaking')}</div>`);
        return;
      }
      ...
      this.setContent(`<div class="insights-error">${i18n.t('insights.analysisFailed')}</div>`);
    }
  }
