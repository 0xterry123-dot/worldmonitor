...
export class LiveNewsPanel extends Panel {
  ...
  private updateLiveIndicator(): void {
    ...
    this.liveBtn.innerHTML = this.isPlaying
      ? `<span class="live-dot"></span>${i18n.t('playback.live')}`
      : `<span class="live-dot paused"></span>${i18n.t('playback.paused')}`;
  ...
  private showOfflineMessage(channel: LiveChannel): void {
    this.content.innerHTML = `
      <div class="live-offline">
        ...
        <div class="offline-text">${channel.name} ${i18n.t('liveNews.notLive')}</div>
        <button class="offline-retry" onclick="this.closest('.panel').querySelector('.live-channel-btn.active')?.click()">${i18n.t('common.retry')}</button>
      </div>
    `;
  }
