...
export class GdeltIntelPanel extends Panel {
  ...
  constructor() {
    super({
      id: 'gdelt-intel',
      title: i18n.t('panels.gdelt-intel'),
      showCount: true,
      trackActivity: true,
      infoTooltip: `<strong>${i18n.t('gdelt.title')}</strong>
        ${i18n.t('gdelt.description')}
        <ul>
          <li>${i18n.t('gdelt.topics')}</li>
          <li>${i18n.t('gdelt.languages')}</li>
          <li>${i18n.t('gdelt.updates')}</li>
        </ul>
        ${i18n.t('gdelt.source')}`,
    });
    this.createTabs();
  ...
  private renderArticles(articles: GdeltArticle[]): void {
    if (articles.length === 0) {
      this.content.innerHTML = `<div class="empty-state">${i18n.t('gdelt.noArticles')}</div>`;
      return;
    }
