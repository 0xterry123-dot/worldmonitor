...
export class NewsPanel extends Panel {
  ...
  private initTranslationListener(): void {
    this.unsubscribeTranslation = translationStore.subscribe(() => {
  ...
  private createTranslateButton(): void {
    ...
    this.translateBtn.className = 'translate-btn';
    this.updateTranslateButton();
    this.translateBtn.addEventListener('click', () => {
      translationStore.toggle();
    });

    // Insert before count element
    const countEl = this.header.querySelector('.panel-count');
    if (countEl) {
      this.header.insertBefore(this.translateBtn, countEl);
    } else {
      this.header.appendChild(this.translateBtn);
    }
  }

  private updateTranslateButton(): void {
    if (!this.translateBtn) return;
    const isEnabled = translationStore.enabled;
    const showOriginal = translationStore.showingOriginal;
    
    if (!isEnabled) {
      this.translateBtn.textContent = i18n.t('newsPanel.translate');
      this.translateBtn.classList.remove('active');
    } else if (showOriginal) {
      this.translateBtn.textContent = i18n.t('newsPanel.showOriginal');
      this.translateBtn.classList.add('active');
    } else {
      this.translateBtn.textContent = i18n.t('newsPanel.showChinese');
      this.translateBtn.classList.add('active');
    }
  }
  ...
  private async refreshTranslation(): Promise<void> {
    this.updateTranslateButton();
    if (!translationStore.enabled) return;
    
    if (this.currentClusters.length > 0) {
      await translationStore.translateClusters(this.currentClusters);
      this.rerender();
    }
  }
  ...
  private rerender(): void {
    ...
    const event = new CustomEvent('newspanel-refresh', { detail: { panelId: this.panelId } });
  ...
  private createSummarizeButton(): void {
    ...
    this.summaryBtn.title = i18n.t('newsPanel.summarizeTitle');
  ...
  private async handleSummarize(): Promise<void> {
    ...
    this.summaryContainer.innerHTML = `<div class="panel-summary-loading">${i18n.t('newsPanel.generating')}</div>`;
    ...
    try {
      ...
      if (result?.summary) {
        ...
        this.summaryContainer.innerHTML = `<div class="panel-summary-error">${i18n.t('newsPanel.couldNotGenerate')}</div>`;
        setTimeout(() => this.hideSummary(), 3000);
      }
    } catch {
      this.summaryContainer.innerHTML = `<div class="panel-summary-error">${i18n.t('newsPanel.summaryFailed')}</div>`;
      setTimeout(() => this.hideSummary(), 3000);
    } finally {
  ...
  private showSummary(summary: string): void {
    ...
    this.summaryContainer.innerHTML = `
      <div class="panel-summary-content">
        ...
        <button class="panel-summary-close" title="${i18n.t('modals.close')}">×</button>
      </div>
    `;
  ...
  private renderClusters(clusters: ClusteredEvent[]): void {
    // Store clusters for translation
    ...
    if (translationStore.enabled) {
      translationStore.translateClusters(sorted).catch(err => {
        console.warn('[NewsPanel] Translation failed:', err);
      });
    }
  ...
  private renderClusterHtml(
    ...
    const topSourcesHtml = otherSources.length > 0
      ? `<span class="also-reported">${i18n.t('newsPanel.alsoReported')}:</span>` + otherSources
          .map(s => {
    ...
    // Get display title (translated if enabled)
    const displayTitle = translationStore.getDisplayTitle(cluster);
    ...
    return `
      <div class="${itemClasses}" ${cluster.monitorColor ? `style="border-left-color: ${escapeHtml(cluster.monitorColor)}"` : ''} data-cluster-id="${escapeHtml(cluster.id)}" data-news-id="${escapeHtml(cluster.primaryLink)}">
        ...
        <a class="item-title" href="${sanitizeUrl(cluster.primaryLink)}" target="_blank" rel="noopener">${escapeHtml(displayTitle)}</a>
  ...
  public destroy(): void {
    ...
    // Unsubscribe from translation store
    if (this.unsubscribeTranslation) {
