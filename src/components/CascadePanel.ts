...
export class CascadePanel extends Panel {
  ...
  constructor() {
    super({
      id: 'cascade',
      title: i18n.t('panels.cascade'),
      showCount: true,
      trackActivity: true,
      infoTooltip: `<strong>${i18n.t('cascade.title')}</strong>
        ${i18n.t('cascade.description')}
        <ul>
          <li>${i18n.t('cascade.cablesPipelines')}</li>
          <li>${i18n.t('cascade.selectInfra')}</li>
          <li>${i18n.t('cascade.showsAffected')}</li>
          <li>${i18n.t('cascade.identifiesRoutes')}</li>
        </ul>
        ${i18n.t('cascade.dataSource')}`,
    });
    this.init();
  }
    ...
    return `
      <div class="cascade-selector">
        ...
        <select class="cascade-select" ${nodes.length === 0 ? 'disabled' : ''}>
          <option value="">${i18n.t('cascade.select')} ${this.filter}...</option>
          ${nodeOptions}
        </select>
        <button class="cascade-analyze-btn" ${!this.selectedNode ? 'disabled' : ''}>
          ${i18n.t('cascade.analyze')}
        </button>
      </div>
    `;
  }
  ...
  private renderCascadeResult(): string {
    ...
    const countriesHtml = countriesAffected.length > 0
      ? countriesAffected.map(c => `
          <div class="cascade-country" style="border-left: 3px solid ${this.getImpactColor(c.impactLevel)}">
            ...
            ${c.affectedCapacity > 0 ? `<span class="cascade-capacity">${Math.round(c.affectedCapacity * 100)}% ${i18n.t('cascade.capacity')}</span>` : ''}
          </div>
        `).join('')
      : `<div class="empty-state">${i18n.t('cascade.noImpacts')}</div>`;

    const redundanciesHtml = redundancies && redundancies.length > 0
      ? `
        <div class="cascade-section">
          <div class="cascade-section-title">${i18n.t('cascade.alternativeRoutes')}</div>
          ${redundancies.map(r => `
    ...
    return `
      <div class="cascade-result">
        ...
        <div class="cascade-section">
          <div class="cascade-section-title">${i18n.t('cascade.countriesAffected')} (${countriesAffected.length})</div>
  ...
  private render(): void {
    ...
    this.content.innerHTML = `
      <div class="cascade-panel">
        ${statsHtml}
        ${this.renderSelector()}
        ${this.cascadeResult ? this.renderCascadeResult() : `<div class="cascade-hint">${i18n.t('cascade.selectHint')}</div>`}
      </div>
    `;
