...
export class EconomicPanel extends Panel {
  ...
  private renderIndicators(): string {
    if (this.fredData.length === 0) {
      return `<div class="economic-empty">${i18n.t('economic.noData')}</div>`;
    }
  ...
  private renderOil(): string {
    if (!this.oilData) {
      return `<div class="economic-empty">${i18n.t('economic.oilNotAvailable')}</div>`;
    }
    ...
    if (metrics.length === 0) {
      return `<div class="economic-empty">${i18n.t('economic.oilNoMetrics')}</div>`;
    }
        ...
        ${metrics.map(metric => {
          ...
          return `
            <div class="economic-indicator oil-metric">
              ...
              <div class="indicator-date">${i18n.t('economic.vsPreviousWeek')}</div>
            </div>
          `;
        }).join('')}
      </div>
  ...
  private renderSpending(): string {
    if (!this.spendingData || this.spendingData.awards.length === 0) {
      return `<div class="economic-empty">${i18n.t('economic.noAwards')}</div>`;
    }
