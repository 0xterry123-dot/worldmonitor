...
export class CIIPanel extends Panel {
  ...
  constructor() {
    super({
      id: 'cii',
      title: i18n.t('panels.cii'),
      showCount: true,
      trackActivity: true,
      infoTooltip: `<strong>${i18n.t('common.methodologyTitle', 'CII Methodology')}</strong>
        ${i18n.t('common.scoreBasedOn', 'Score (0-100) per country based on:')}
        <ul>
          <li>40% ${i18n.t('common.baselineGeopoliticalRisk', 'baseline geopolitical risk')}</li>
          <li><strong>U</strong>${i18n.t('ciipanel.components.unrest')}: ${i18n.t('common.unrestDesc', 'protests, fatalities, internet outages')}</li>
          <li><strong>S</strong>${i18n.t('ciipanel.components.security')}: ${i18n.t('common.securityDesc', 'military flights/vessels over territory')}</li>
          <li><strong>I</strong>${i18n.t('ciipanel.components.information')}: ${i18n.t('common.informationDesc', 'news velocity and focal point correlation')}</li>
          <li>${i18n.t('common.hotspotProximityBoost', 'Hotspot proximity boost (strategic locations)')}</li>
        </ul>
        <em>${i18n.t('common.usiValuesShow', 'U:S:I values show component scores.')}</em>
        ${i18n.t('common.focalPointDetectionNote', 'Focal Point Detection correlates news entities with map signals for accurate scoring.')}`,
    });
    this.showLoading(i18n.t('status.scanningIntelligenceFeeds'));
  }
    ...
    return `
      <div class="cii-country" data-code="${escapeHtml(country.code)}">
        <div class="cii-header">
          ...
          ${trend}
          <button class="cii-share-btn" data-code="${escapeHtml(country.code)}" data-name="${escapeHtml(country.name)}" title="${i18n.t('countryIntel.shareStory')}"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M4 12v7a2 2 0 002 2h12a2 2 0 002-2v-7"/><polyline points="16 6 12 2 8 6"/><line x1="12" y1="2" x2="12" y2="15"/></svg></button>
        </div>
        <div class="cii-bar-container">
          <div class="cii-bar" style="width: ${barWidth}%; background: ${color};"></div>
        </div>
        <div class="cii-components">
          <span title="${i18n.t('ciipanel.components.unrest')}">U:${country.components.unrest}</span>
          <span title="${i18n.t('ciipanel.components.conflict')}">C:${country.components.conflict}</span>
          <span title="${i18n.t('ciipanel.components.security')}">S:${country.components.security}</span>
          <span title="${i18n.t('ciipanel.components.information')}">I:${country.components.information}</span>
        </div>
      </div>
    `;
  }
  ...
  public async refresh(forceLocal = false): Promise<void> {
    ...
    try {
      ...
      if (withData.length === 0) {
        this.content.innerHTML = `<div class="empty-state">${i18n.t('status.noInstabilitySignals')}</div>`;
        return;
      }
