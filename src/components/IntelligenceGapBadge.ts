...
export class IntelligenceFindingsBadge {
  ...
  constructor() {
    ...
    this.badge.title = i18n.t('intelligenceGapBadge.title');
  ...
  private renderDropdown(): void {
    if (this.findings.length === 0) {
      this.dropdown.innerHTML = `
        <div class="findings-header">
          <span class="header-title">${i18n.t('intelligenceGapBadge.title')}</span>
          <span class="findings-badge none">${i18n.t('intelligenceGapBadge.monitoring')}</span>
        </div>
        <div class="findings-content">
          <div class="findings-empty">
            <span class="empty-icon">📡</span>
            <span class="empty-text">${i18n.t('intelligenceGapBadge.scanning')}</span>
          </div>
        </div>
      `;
      return;
    ...
    this.dropdown.innerHTML = `
      ...
      <div class="findings-content">
        ...
        ${moreCount > 0 ? `<div class="findings-more">+${moreCount} ${i18n.t('intelligenceGapBadge.more')}</div>` : ''}
      </div>
    `;
  }
