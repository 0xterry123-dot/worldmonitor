...
export class WindowedList<T> {
  ...
  private scrollRAF: number | null = null;
  private idToIndex?: Map<string, number>; // For refreshItem by ID (requires items to have id property)
  ...
  setItems(items: T[]): void {
    ...
    // Build ID to index map for refreshItem (if items have 'id' property)
    // We'll try to detect the id property dynamically
    const firstItem = items[0];
    if (firstItem && typeof firstItem === 'object' && 'id' in firstItem) {
      this.idToIndex = new Map();
      items.forEach((item, idx) => {
        const id = (item as any).id;
        if (id) this.idToIndex!.set(String(id), idx);
      });
    } else {
      this.idToIndex = undefined;
    }
  ...
  /**
   * Refresh a specific item by its index (for WindowedList only)
   */
