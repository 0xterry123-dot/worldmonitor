      ...
      <div class="etf-flows-container">
        <div class="etf-summary ${dirClass}">
          <div class="etf-summary-item">
            <span class="etf-summary-label">${i18n.t('etf.netFlow')}</span>
            <span class="etf-summary-value ${dirClass}">${escapeHtml(s.netDirection)}</span>
          </div>
          <div class="etf-summary-item">
            <span class="etf-summary-label">${i18n.t('etf.estFlow')}</span>
            <span class="etf-summary-value">$${formatVolume(Math.abs(s.totalEstFlow))}</span>
          </div>
          <div class="etf-summary-item">
            <span class="etf-summary-label">${i18n.t('etf.totalVol')}</span>
            <span class="etf-summary-value">${formatVolume(s.totalVolume)}</span>
          </div>
          <div class="etf-summary-item">
            <span class="etf-summary-label">${i18n.t('etf.etfs')}</span>
        ...
        <div class="etf-table-wrap">
          <table class="etf-table">
            <thead>
              <tr>
                <th>${i18n.t('etf.ticker')}</th>
                <th>${i18n.t('etf.issuer')}</th>
                <th>${i18n.t('etf.estFlow')}</th>
                <th>${i18n.t('etf.volume')}</th>
                <th>${i18n.t('etf.change')}</th>
              </tr>
            </thead>
            <tbody>${rows}</tbody>
          </table>
